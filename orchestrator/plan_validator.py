...
class PlanValidator:
    ...
    def validate(self, plan_data: dict, agent_registry: Any = None) -> dict:
        ...
        # Publishing node dependency check (rule 19)
        edges = self._check_publishing_dependencies(nodes, edges, node_ids)
        plan_data["edges"] = edges
    ...
    # Keywords that identify a publishing/deployment node in task descriptions.
    _PUBLISHING_KEYWORDS = (
        "push pr", "create pull request", "create a pr",
        "deploy", "release", "publish", "notify",
        "git push", "push branch", "open a pr",
    )

    def _check_publishing_dependencies(
        self,
        nodes: list[dict],
        edges: list[dict],
        node_ids: set[str],
    ) -> list[dict]:
        """Ensure publishing/deployment nodes depend on all impl+eval nodes (#1124).
        ...
        for node in nodes:
            ...
            if not nid:
                continue
            task = (node.get("task") or "").lower()
            if any(kw in task for kw in self._PUBLISHING_KEYWORDS):
                publishing_ids.append(nid)
            elif any(kw in nid.lower() for kw in ("push_pr", "deploy", "release")):
                publishing_ids.append(nid)
        ...
        for pub_id in publishing_ids:
            ...
            for dep_id in missing:
                if (dep_id, pub_id) not in edge_set:
                    ...
                    new_edges.append(new_edge)
                    edge_set.add((dep_id, pub_id))
                    self.warnings.append(
                        f"Publishing node '{pub_id}' missing dependency on "
                        f"'{dep_id}' — auto-added hard dependency (rule 19: "
                        f"publishing nodes must be terminal)."
                    )
