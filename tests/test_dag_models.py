...
class TestDAGNodeHeartbeat:
    """Test DAGNode heartbeat recording — returns new instance (immutability)."""

    def test_record_heartbeat_updates_timestamp(self):
        node = DAGNode(id="n1", agent_type="gen", task_description="Test")
        before = datetime.now(timezone.utc)
        updated = node.record_heartbeat()
        after = datetime.now(timezone.utc)
        assert before <= updated.last_heartbeat_at <= after
    ...
    def test_record_heartbeat_increments_count(self):
        ...
        updated2 = updated.record_heartbeat()
        assert updated2.heartbeat_count == 2
    ...
    def test_record_heartbeat_recovers_health(self):
        ...
        updated = node.record_heartbeat()
        assert updated.health_status == NodeHealth.HEALTHY

    def test_record_heartbeat_returns_new_instance(self):
        """Verify immutability: record_heartbeat returns a new DAGNode."""
        node = DAGNode(id="n1", agent_type="gen", task_description="Test")
        updated = node.record_heartbeat()
        assert updated is not node
        assert node.heartbeat_count == 0  # Original unchanged
        assert updated.heartbeat_count == 1


class TestDAGNodeHealthCheck:
    """Test DAGNode health checking logic — returns (health, new_node) tuple."""
    ...
    def test_healthy_node_within_interval(self):
        ...
        health, updated = node.check_health(heartbeat_interval_sec=5.0)
    ...
    def test_unhealthy_after_miss_threshold(self):
        ...
        health, updated = node.check_health(heartbeat_interval_sec=5.0, miss_threshold=3)
    ...
    def test_missed_below_threshold(self):
        ...
        health, updated = node.check_health(heartbeat_interval_sec=5.0, miss_threshold=3)
        assert health == NodeHealth.MISSED
    ...
    def test_non_running_node_returns_current(self):
        ...
        health, updated = node.check_health()
        assert health == NodeHealth.HEALTHY  # Default health status
    ...
    def test_no_heartbeat_no_start_returns_current(self):
        ...
        health, updated = node.check_health()
        assert health == NodeHealth.HEALTHY  # Cannot determine elapsed

    def test_check_health_returns_new_instance(self):
        """Verify immutability: check_health returns a new DAGNode."""
        old = datetime.now(timezone.utc) - timedelta(seconds=30)
        node = DAGNode(
            id="n1", agent_type="gen", task_description="Test",
            status=NodeStatus.RUNNING,
        ...
        health, updated = node.check_health(heartbeat_interval_sec=5.0, miss_threshold=3)
        assert updated is not node
        assert node.missed_heartbeats == 0  # Original unchanged
        assert updated.missed_heartbeats > 0
