...
and is configurable via WEAVE_RUN_TIMEOUT_SEC env var or CLI --timeout.
"""
import inspect
...
class TestRunTimeoutDefault:
    ...
    def test_env_var_overrides_default(self, monkeypatch):
        """WEAVE_RUN_TIMEOUT_SEC env var overrides the default."""
        monkeypatch.setenv("WEAVE_RUN_TIMEOUT_SEC", "3600")
        config = WeaveConfig()
        assert config.run_timeout_sec == 3600

    def test_explicit_value_overrides_env(self):
        """Explicit constructor value takes precedence."""
        config = WeaveConfig(run_timeout_sec=900)
