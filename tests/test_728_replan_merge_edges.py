...
def _make_full_dag() -> DAG:
    ...
    # Edges
    dag = dag.add_edge("plan", "impl_foundation")
    for nid in ["impl_a", "impl_b", "impl_c"]:
        dag = dag.add_edge("impl_foundation", nid)
        dag = dag.add_edge(nid, "tests")
    dag = dag.add_edge("tests", "eval")
    return dag
...
class TestMergePreservesEdges:
    ...
    def test_old_edges_preserved_for_carried_nodes(self):
        ...
        new_dag = new_dag.add_edge("plan", "impl_foundation_v2")
    ...
    def test_no_duplicate_edges(self):
        ...
        old_dag = old_dag.add_node(DAGNode(id="a", agent_type="generator", task_description="a"))
        old_dag = old_dag.add_node(DAGNode(id="b", agent_type="generator", task_description="b"))
        old_dag = old_dag.add_edge("a", "b")

        new_dag = DAG(reasoning="new")
        new_dag = new_dag.add_node(DAGNode(id="a", agent_type="generator", task_description="a"))
        new_dag = new_dag.add_node(DAGNode(id="b", agent_type="generator", task_description="b"))
        new_dag = new_dag.add_edge("a", "b")
